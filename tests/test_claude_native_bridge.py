...
def test_ensure_secure_dir_skips_posix_uid_check_on_windows(
    ...
    ``os.getuid`` is absent and ``st_uid`` is unpopulated (0) on Windows, so the
    check always fails and fail-closes the omnigent MCP relay. Gated on the
    availability of POSIX uid semantics (``os.getuid``), a foreign-uid directory
    no longer raises there.
    """
    root = tmp_path / "bridgeroot"
    child = root / "child"
    child.mkdir(parents=True)
    monkeypatch.setattr(claude_native_bridge, "_trusted_parent_for_bridge_dir", lambda _t: root)
    # No ``os.getuid`` — simulate Windows, where Python exposes no uid model.
...
def test_ensure_secure_dir_uid_mismatch_still_raises_on_posix(
    ...
    child.mkdir(parents=True)
    monkeypatch.setattr(claude_native_bridge, "_trusted_parent_for_bridge_dir", lambda _t: root)
    # ``os.getuid`` present — POSIX behavior, uid ownership is enforced.
    monkeypatch.setattr(claude_native_bridge.os, "getuid", lambda: 1000, raising=False)
    monkeypatch.setattr(claude_native_bridge.os, "lstat", lambda _p: _fake_dir_stat(4242))
...
def test_ensure_secure_dir_symlink_rejected_on_both_platforms(
    ...
    for has_getuid in (True, False):
        # POSIX (``os.getuid`` present) and Windows (absent) alike.
        if has_getuid:
            monkeypatch.setattr(claude_native_bridge.os, "getuid", lambda: 1000, raising=False)
        else:
            monkeypatch.delattr(claude_native_bridge.os, "getuid", raising=False)
